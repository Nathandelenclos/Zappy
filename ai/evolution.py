...
def evolve_in_group(client_socket):
    ...
    printGreen("Number of players: " + str(
        myGameData.available["lvl" + str(myGameData.lvl)]))
    if myGameData.available["lvl" + str(myGameData.lvl)] >= int(
...
def gathering_mode(client_socket):
    ...
    while myGameData.nb_arrived < int(
            myGameData.evolution_infos["lvl" + str(myGameData.lvl)][0]):
        client_socket.send(
            ("Broadcast " + myGameData.team_name + " gather\n").encode())
